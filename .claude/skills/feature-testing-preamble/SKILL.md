...
- The app is authenticated. If `mcp__playwright__browser_navigate` lands on a sign-in screen, stop and ask the user to refresh authentication before continuing.

## Steps

1. Read `.dev-port` if present to get `<port>`. If absent, use the known dev URL for the running session.
2. `mcp__playwright__browser_navigate` to `http://localhost:<port>`.
...
- Does not reset app state or clear existing records. Use `mcp__playwright__browser_close` if you need a clean browser state.
